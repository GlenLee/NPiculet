...
namespace NPiculet.Logic.Business
{
	...
	/// 附件处理业务
	/// </summary>
	public partial class AttachmentBus : IBusiness
	{
		/// <summary>
		/// 获取文件列表
		/// </summary>
		...
		public List<bas_attachment> GetFileList(string moduleCode, string moduleId, string sourceCode, string sourceId, int parentId)
		{
			using (NPiculetEntities db = new NPiculetEntities()) {
				var query = (from a in db.bas_attachment
					where a.ModuleCode == moduleCode && a.ModuleId == moduleId && a.SourceCode == sourceCode && a.SourceId == sourceId && a.ParentId == parentId
