...
namespace NPiculet.Logic.Business
{
	...
	public partial class SysAuthorizationBus
	{
		...
		{
			string sql = @"SELECT DISTINCT sm.`Id`, sm.`Name`, sm.`ParentId`, sm.`RootId`, sm.`Path`, sm.`Depth`, sm.`OrderBy`, 1 AS `IsAuth` FROM npc_sys_menu sm
	INNER JOIN (SELECT m.`Id`, m.`Name`, m.`ParentId`, m.`RootId`, m.`Path`, m.`Depth` FROM npc_sys_menu m
WHERE m.`IsDel`=0 and m.`IsEnabled`=1 and EXISTS(SELECT * FROM npc_sys_authorization a WHERE m.`Id`=a.`FunctionId`";
		...
		{
			string sql = @"SELECT DISTINCT m.`Id`, m.`Name`, m.`ParentId`, m.`RootId`, m.`Path`, m.`Depth`, m.`OrderBy`, Count(a.`Id`) AS `IsAuth` FROM npc_sys_menu m
	LEFT JOIN npc_sys_authorization a ON m.`Id`=a.`FunctionId`";

			sql += string.Format(" and a.`TargetType`='{0}' and a.`TargetId`='{1}'", targetType, targetId);

			sql += " WHERE m.`IsDel`=0";
			sql += " GROUP BY m.`Id`, m.`Name`, m.`ParentId`, m.`RootId`, m.`Path`, m.`Depth`";
...
FROM npc_sys_user_info u
	INNER JOIN npc_sys_link_user_role l ON u.`Id`=l.`UserId` AND u.`IsDel`=0
	INNER JOIN npc_sys_role_info r ON l.`RoleId`=r.`Id` AND r.`IsDel`=0";
...
FROM npc_sys_user_info u
	INNER JOIN npc_sys_link_user_org l ON u.`Id`=l.`UserId` AND u.`IsDel`=0
	INNER JOIN npc_sys_org_info o ON l.`OrgId`=o.`Id` AND o.`IsDel`=0";
